...
public:
    ...
    __nocopy__(fs_based_logger) public : virtual void debug(const std::string& log_line)
    {
        if (level_ <= 0)
        {
            ...
            [this, self](const asio::error_code& err, size_t) -> void
            {
                if (!err)
                {
                    ...
                    if (data_size < 0 || data_size > 0x1000000)
                    {
                        l_->warn(
                            lstrfmt(
                                "bad log data size in the header %d, stop this session to protect further corruption")
                                .fmt(data_size));
                        this->stop();
                        return;
                    }
                    ...
                    asio::async_read(
                        ...
                        [self](const asio::error_code& err, size_t n) { self->read_log_data(err, n); });
                }
                else
                {
        ...
        try
        {
            ...
            else
            {
                ...
                asio::async_write(
                    socket_,
                    std::move(buffer),
                    [this, self, buf = std::move(resp_buf)](asio::error_code err_code, size_t) -> void
                    {
                        if (!err_code)
                        {
                            this->header_->pos(0);
                        ...
                        {
                            this->l_->err(
                                lstrfmt("failed to send response to peer due to error %d").fmt(err_code.value()));
                            this->stop();
                        }
                    });
            }
...
private:
    void start()
    {
        ...
        ptr<rpc_session> session(
            cs_new<rpc_session>(io_svc_, handler_, l_, [self](const ptr<rpc_session>& s) { self->remove_session(s); }));
        acceptor_.async_accept(
            session->socket(),
            [self, this, session](const asio::error_code& err) -> void
            {
                if (!err)
                {
    ...
    virtual void send(ptr<req_msg>& req, rpc_handler& when_done) __override__
    {
        ...
        if (!socket_.is_open())
        {
            ...
            resolver_.async_resolve(
                q,
                [self, this, req, when_done](std::error_code err, asio::ip::tcp::resolver::iterator itor) mutable
                {
                    if (!err)
                    {
                        asio::async_connect(
                    ...
                    else
                    {
                        ...
                        ptr<rpc_exception> except(cs_new<rpc_exception>(
                            lstrfmt("failed to resolve host %s due to error %d").fmt(host_.c_str(), err.value()), req));
        ...
        else
        {
            ...
            asio::async_write(
                socket_,
                std::move(buffer),
                [self, req_msg = req, when_done, buf = std::move(req_buf)](
                    std::error_code err, size_t bytes_transferred) mutable -> void
...
private:
        ...
        size_t /* bytes_transferred */)
    {
        ...
        if (!err)
        {
            ...
            asio::async_read(
                socket_,
                std::move(buffer),
                [self, req_msg = req, when_done, rbuf = std::move(resp_buf)](
                    std::error_code err, size_t bytes_transferred) mutable -> void
        ...
        else
        {
            ptr<resp_msg> rsp;
            ptr<rpc_exception> except(
                cs_new<rpc_exception>(sstrfmt("failed to send request to remote socket %d").fmt(err.value()), req));
            socket_.close();
            when_done(rsp, except);
        }
    }
        ...
        size_t /* bytes_transferred */)
    {
        ...
        else
        {
            ptr<resp_msg> rsp;
            ptr<rpc_exception> except(
                cs_new<rpc_exception>(sstrfmt("failed to read response to remote socket %d").fmt(err.value()), req));
            socket_.close();
            when_done(rsp, except);
        }
    }
...
void asio_service_impl::run()
{
    ...
    {
        std::thread t([self = shared_from_this()]() mutable { self->worker_entry(); });
        t.detach();
    }
}
...
void cornerstone::impls::fs_based_logger::write_log(const std::string& level, const std::string& log_line)
{
    ...
    std::string line(sstrfmt("%d/%d/%d %d:%d:%d.%d\t[%d]\t%s\t")
                         .fmt(
                             tm->tm_mon + 1,
                             tm->tm_mday,
                             tm->tm_year + 1900,
...
asio_service::asio_service() : impl_(cs_new<asio_service_impl>())
{
    impl_->run();
}
...
ptr<rpc_client> asio_service::create_client(const std::string& endpoint)
{
    ...
    return cs_new<asio_rpc_client>(impl_->io_svc_, hostname, port);
}
